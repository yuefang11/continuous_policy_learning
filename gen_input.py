...
X_train, X_test, y_train, y_test, T_train, T_test, X_policy_train, X_policy_test = train_test_split(
    X, y, T, X_policy, test_size=0.2, random_state=42
)
...
cost = 200
params = {'t_lo': t_lo, 't_hi': t_hi, 'thres': 0.01}
data_estimation = {'y': y_train,  'x':  X_train, 'x_pol': X_policy_train, 'T': T_train,  'cost': cost, 'A_hat': A_hat, 'r_hat': r_hat, 't_lo': t_lo, 't_hi': t_hi}
data_testing = {'y': y_test,  'x':  X_test, 'x_pol': X_policy_test, 'T': T_test,  'cost': cost, 'A_hat': A_hat, 'r_hat': r_hat, 't_lo': t_lo, 't_hi': t_hi}

input = {'data_estimation': data_estimation, 'data_testing': data_testing}
# Save input dictionary to pickle file
date_time = datetime.now().strftime("%m%d")
with open(f'input_{date_time}.pkl', 'wb') as f:
    pickle.dump(input, f)
